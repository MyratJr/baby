...
class PDFView(PDFTemplateView):
    cmd_options = {
        ...
        'quiet': False, 'enable-local-file-access': True
    }
    ...
    #     try:
    #         resume = Resume.objects.get(user=self.request.user)
    #         if resume.template and resume.template.name:
    #             parts = resume.template.name.split(" ")
    #             template_name = f'cvbuilder/resumes/{parts[0].lower()}.html' if len(parts) > 0 else default_template
    #             color = parts[1].lower() if len(parts) > 1 else default_color
    #             return {'template_name': template_name, 'color': color}
    #     except Exception as e:
    #         logger.warning(f"Error in template or color selection: {e}")
...
class MainPage(LoginRequiredMixin, View):
    ...
    def get(self, request):
        try:
            ...
            print("Resume is so unexpectedly!!!")
